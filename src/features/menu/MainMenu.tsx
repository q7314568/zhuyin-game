...
const MainMenu: React.FC<MainMenuProps> = ({ onStart, onLibrary, onSoundHunter }) => {
    const [enableTimer, setEnableTimer] = React.useState(true);
    // 選單視圖狀態：'main' (主選單) 或 'listening_difficulty' (聽音練習難度選擇)
    const [menuView, setMenuView] = React.useState<'main' | 'listening_difficulty'>('main');

    // 聽音練習難度選擇視圖
    if (menuView === 'listening_difficulty') {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen w-full bg-blue-50 p-4">
                <h1 className="text-3xl md:text-4xl font-bold mb-6 md:mb-8 text-blue-600">選擇難度</h1>
                ...
                <div className="space-y-3 md:space-y-4 w-full flex flex-col items-center">
                    ...
                    <button
                        ...
                        className="w-full max-w-xs sm:w-64 py-3 bg-gray-500 text-white rounded-lg shadow-md hover:bg-gray-600 transition text-lg md:text-xl font-semibold"
                    >
                        返回主選單
                    </button>
                </div>
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen w-full bg-blue-50 p-4">
            ...
            <div className="space-y-3 md:space-y-4 w-full flex flex-col items-center">
                {/* 進入聽音練習選單 */}
                <button
                    onClick={() => setMenuView('listening_difficulty')}
                ...
                {/* 進入注音獵人遊戲 */}
                <button
                    onClick={onSoundHunter}
                    className="w-full max-w-xs sm:w-64 py-3 bg-indigo-500 text-white rounded-lg shadow-md hover:bg-indigo-600 transition text-lg md:text-xl font-semibold"
                >
                    注音獵人 (Sound Hunter)
                </button>
                {/* 進入注音庫 */}
                <button
                    onClick={onLibrary}
                    className="w-full max-w-xs sm:w-64 py-3 bg-teal-500 text-white rounded-lg shadow-md hover:bg-teal-600 transition text-lg md:text-xl font-semibold"
                >
                    注音庫
                </button>
            </div>
